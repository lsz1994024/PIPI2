...
public class WTNode {
    ...
    private WTNode leftNode;
    private WTNode rightNode;
    private HashMap<Character, Boolean> characterBitMap;

    public WTNode(ArrayList<Character> S, ArrayList<Character> sortedAlphabet){
        ...
        preOrder = new PreOrder(B);

        leftNode = new WTNode(leftS, sortedLeftAlphabet);
        rightNode = new WTNode(rightS, sortedRightAlphabet);
    }
    ...
    public int Occ(int i, char c){
        ...
        if (characterBitMap.get(c)) {
            return rightNode.Occ(preOrder.rank1(i - 1), c);
        } else {
            return leftNode.Occ(i - preOrder.rank1(i - 1), c);
        }
    }
}
